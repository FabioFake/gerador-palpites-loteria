...
export class DecididorQualProbabilidadeService {
  ...
  public criarArrayPorcentagensParaDecisao(porcentagens: Array<number>): Array<number>{
    const porcentagensParaDecremento: Array<number> = [...porcentagens];
    const porcentagensParaDecisao: Array<number> = [];
    for (let i = 0; i<100; i ++){
      let indicePorcentagem = this.acharIndiceProximaPorcentagemParaArrayDecisao(porcentagensParaDecremento);
  ...
  private acharIndiceProximaPorcentagemParaArrayDecisao(porcentagemParaDecremento: Array<number>): number{
    if(porcentagemParaDecremento.length === 0){
      return -1;
    }
    let indiceSorteado: number = Math.floor(Math.random() * porcentagemParaDecremento.length);
    if(porcentagemParaDecremento[indiceSorteado] === 0){
      porcentagemParaDecremento = porcentagemParaDecremento.filter((numero) => numero > 0 );
      indiceSorteado = this.acharIndiceProximaPorcentagemParaArrayDecisao(porcentagemParaDecremento);
    }
    if(indiceSorteado > -1){
